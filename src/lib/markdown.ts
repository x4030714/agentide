/**
 * The small slice of Markdown an agent actually writes, parsed into blocks.
 *
 * Not a Markdown library. Two reasons, and neither is size:
 *
 * 1. **No HTML anywhere.** A library returns an HTML string, which means
 *    `dangerouslySetInnerHTML` and a sanitiser to go with it -- an injection surface
 *    opened for text that is, by definition, generated by a language model reading files
 *    from disk. This produces plain data that the renderer turns into React elements, so
 *    there is no string of markup at any point.
 * 2. **The output has to look like this app.** A code fence here is the same recessed
 *    mono block the tool details use, not a library's idea of a `<pre>`. Owning the
 *    parse is what makes that a rendering decision rather than a stylesheet fight.
 *
 * What is deliberately not supported: tables, block quotes, images, HTML, reference
 * links, nested lists. Anything unrecognised stays as the literal text the model wrote,
 * which is the honest failure -- a stray `|` reads as a stray `|`, not as a broken table.
 */
...
/**
 * Inline markers, scanned left to right.
 *
 * Code is matched first and its contents are never looked at again, so `**` inside a
 * backtick span stays literal -- which is what makes `` `**ptr` `` render as C rather
 * than as a bold run that never closes.
 */
