...
test("a server the workspace also names is the workspace's, not the user's", () => {
  const { home, cwd } = tree(
    config({ ida: { command: "python", args: ["user.py"] }, blender: { command: "uvx" } }),
    config({ ida: { command: "python", args: ["project.py"] } }),
  );

  assert.deepEqual(loadMcpServers(cwd, home), {
    ida: { command: "python", args: ["project.py"] },
    blender: { command: "uvx" },
  });
});

test("a user-level server applies in a workspace that has no config at all", () => {
...
test("no config files anywhere is silent, not an error", () => {
  const { home, cwd } = tree(null, null);
  const { value, stderr } = capture(() => loadMcpServers(cwd, home));

  assert.deepEqual(value, {});
  assert.equal(stderr, "");
});

test("a disabled entry is dropped rather than passed with a flag", () => {
  const { home, cwd } = tree(
    config({
...
test("a disabled entry in the workspace overrides an enabled one from the user file", () => {
...
test("a disabled entry that repeats its transport is still just off", () => {
...
test("an entry that is only a disabled flag is off, not a broken config", () => {
...
test("note and disabled never reach the SDK", () => {
  const { home, cwd } = tree(
    config({
      ida: {
        command: "python",
...
test("an entry keyed agentide is refused, so the IDE's own tools cannot be shadowed", () => {
...
test("a file that is not valid JSON costs its servers and not the turn", () => {
...
test("a malformed entry is skipped by name and the rest of the file still loads", () => {
  const { home, cwd } = tree(
    config({
...
test("sse and http entries keep their url and headers", () => {
...
test("an edit to the config is picked up without restarting the process", () => {
  ...
  assert.deepEqual(loadMcpServers(cwd, home), { blender: { command: "uvx" } });
  writeFileSync(
...
test("a ${VAR} in a header is replaced from the environment", () => {
  const { home, cwd } = tree(
    config({
      github: {
        type: "http",
...
test("a ${VAR} that is not set drops the server instead of sending the literal", () => {
...
test("a warning about a missing variable never prints its value", () => {
  const { home, cwd } = tree(
    config({
  ...
  try {
    const { stderr } = capture(() => loadMcpServers(cwd, home));
